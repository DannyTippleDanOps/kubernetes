...
func (f fakeAPI) VolumeDelete(namespace string, ref string) error {
	return nil
}
...
func TestAttachVolume(t *testing.T) {
	...
	_, err = os.Create(filepath.Join(tmpDir, testVolUUID))
	if err != nil {
		t.Errorf("could not create test file, error: %v", err)
	}
	// devicePath, err := util.AttachVolume(mounter)
	//
	// Expecting error:
	// if err != nil {
	// 	t.Errorf("AttachVolume() returned error: %v", err)
	// }
	// if devicePath == "" {
	// 	t.Error("AttachVolume() returned empty devicePath")
	// }
}
